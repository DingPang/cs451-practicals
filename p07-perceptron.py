...
import math
import warnings
warnings.filterwarnings("ignore") # I ignored user warnings generated by plt.tight_layout.
...
# Normalization doesn't improve decision trees, and it really shines when we are solving equations.
# For decision trees, it is going to make similar "cuts" as before
...
# My conclusion is that Normalization definetely improves performance, but I only see a minor
# Improvement from using variance/std to min/max. Thus, I think min/max is best here, but
# "var" could be good for other datasets. (We definetly should use a Normalization)
norm = "max"
if norm == "var":
...
# I think SK uses regular percepton, because the graph really doesn't show that it is averaged
skP = Perceptron()
...
for iter in tqdm(range(1000)):
    ...
    learning_curves["skMLP"].add_sample(mlp, X_train, y_train, X_vali, y_vali)
print("skMLP. Train-Accuracy: {:.3}".format(mlp.score(X_train, y_train)))
print("skMLP. Vali-Accuracy: {:.3}".format(mlp.score(X_vali, y_vali)))

## TODO Exploration 1B: Try another Linear Model
sgdc = SGDClassifier()
print("Train sklearn-SGDClassifier (sgdc)")
for iter in tqdm(range(1000)):
    sgdc.partial_fit(X_train, y_train, classes=(0, 1))
    learning_curves["skSGDC"].add_sample(sgdc, X_train, y_train, X_vali, y_vali)
print("skSGDC. Train-Accuracy: {:.3}".format(sgdc.score(X_train, y_train)))
print("skSGDC. Vali-Accuracy: {:.3}".format(sgdc.score(X_vali, y_vali)))


## TODO Think: Why can't we make a graph like this for DecisionTreeClassifier?
# Because dtree is not a linear model, so it doesn't have a partial fit function
# I don't think this type of incremental learning is going to help dtree.
# For each iter, I think dtree is already trying to find the best "cut".
# This is an interesting post: https://stats.stackexchange.com/questions/305021/incremental-learning-with-decision-trees-scikit-learn
...
# I put all plots all in one figure, so it is easier for me to look at
# I also ignored user warnings generated by tight_layout.
# Just max the window size, and everythin is looking great
num_total = len(learning_curves.items())
sqrt_num = math.ceil(math.sqrt(num_total))
id = 1
for key, dataset in learning_curves.items():
    xs = np.array(list(range(len(dataset.train))))
    plt.subplot(sqrt_num, sqrt_num, id)
    # line-plot:
